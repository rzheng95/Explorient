	...
	public InformationCenter() {
		...
		textFieldSearch.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent arg0) {
				...
				if(column.equals("State/Province"))
				{
					column = "State";
				}
				try{
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String id = null, code = null, name = null;
				// grab column names
				try {
					String query = "SELECT * FROM "+table;
					PreparedStatement pst = connection.prepareStatement(query);
				...
				try{
					...
					if(!table.equals("Hotels"))
						query = "Update "+table+" set "+id+"='"+EXPHLAID+"' ,"+code+"='"+textFieldCode.getText()+"',"
								+ " "+name+"='"+textFieldName.getText()+"' ,Contact='"+textFieldContact.getText()+"' ,LocalPhone='"+textFieldLocalPhone.getText()
		...
		btnCreate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				{
					lblWarningMessage.setText(tableNoS+" Info Incomplete! The following textfields must be provided: Name, Contact, Local Phone, Street, City, and Country.");								
					lblWarningMessage.setForeground(Color.red);
					return;
				}
				if(table.equals("Hotels") && textFieldVendor.getText().equals(""))
				{
					lblWarningMessage.setText("Vendor incomplete!");
					return;
				}
				...
				try{				
					...
					String temp = null;
					if(table.equals("Vendors"))
						temp = "LandService";
					else
						temp = tableNoS;
					HLAID = rs.getInt(temp);	
					HLAID++;
					stmt.close();
					rs.close();
				}catch(Exception e1){e1.printStackTrace();}	
				...
				try{					
					...
					if(!id.equals("HotelID"))
						query = "insert into "+table+" ("+id+","+code+","+name+",Contact,LocalPhone,Fax,Street,City,State,Country,Zipcode) values (?,?,?,?,?,?,?,?,?,?,?)";				
					else
						query = "insert into "+table+" ("+id+","+code+","+name+",Contact,LocalPhone,Fax,Street,City,State,Country,Zipcode,Vendor) values (?,?,?,?,?,?,?,?,?,?,?,?)";
					...
					if(id.equals("HotelID"))
						pst.setString(12, textFieldVendor.getText());
				...
				if(table.equals("Vendors"))
					...
					temp = tableNoS;
				try{				
					String query = "Update IDs set EXPID='"+1+"' , "+temp+"='"+HLAID+"' where EXPID='"+1+"'  ";
					PreparedStatement pst = connection.prepareStatement(query);
					pst.execute();								
					pst.close();
					}catch(Exception e1){
		...
		lblWarningMessage.setBounds(10, 736, 899, 36);
